...
const AccommodationConfirmationInvoicePage = observer(({ match }) => {
    const [invoice, setInvoice] = useState(null);

    useEffect(() => {
        API.get({
            url: API.BOOKING_INVOICE(match?.params?.id),
            success: setInvoice
        });
    }, []);
    ...
    return (
        <div className="invoice">
            ...
            <div className="buttons no-print">
                ...
                <button className="button" onClick={showSendModal}>{t("Send Invoice")}</button>
            </div>
